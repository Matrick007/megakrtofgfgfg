...
def init_db():
    ...
    with get_db_connection() as conn:
        ...
        try:
            conn.execute("ALTER TABLE group_messages ADD COLUMN deleted_for TEXT DEFAULT ''")
            conn.commit()
        except sqlite3.OperationalError:
            pass

        try:
            conn.execute("ALTER TABLE channel_messages ADD COLUMN deleted_for TEXT DEFAULT ''")
            conn.commit()
        except sqlite3.OperationalError:
            pass
...
def get_channel_messages(channel_id, offset=0, limit=50, newest_first=True):
    """
    ...
    Args:
        ...
        newest_first: если True - новые сообщения первыми, если False - старые первыми
    """
    with get_db_connection() as conn:
        # Для новых сообщений (newest_first=True) - сортировка по убыванию
        # Для старых (пагинация) - сортировка по возрастанию
        ...
        # Если загружаем новые (newest_first=True), переворачиваем для правильного отображения
        if newest_first:
            result.reverse()
...
# === Функции для работы с предложками каналов ===

def create_channel_suggestion(channel_id, user_id, content):
    """
    Создать предложение поста для канала

    Args:
        channel_id (int): ID канала
        user_id (int): ID пользователя, который предлагает пост
        content (str): Текст предложения

    Returns:
        int: ID созданного предложения
    """
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO channel_suggestions (channel_id, user_id, content, status)
            VALUES (?, ?, ?, 'pending')
...
def get_channel_suggestions(channel_id):
    """
    Получить все предложения для канала

    Args:
        channel_id (int): ID канала

    Returns:
        list: Список предложений с информацией о пользователях
    """
    with get_db_connection() as conn:
        suggestions = conn.execute("""
            SELECT cs.*, u.username, u.avatar, c.name as channel_name
            FROM channel_suggestions cs
            JOIN users u ON cs.user_id = u.id
            JOIN channels c ON cs.channel_id = c.id
            WHERE cs.channel_id = ? AND cs.status = 'pending'
            ORDER BY cs.created_at DESC
        """, (channel_id,)).fetchall()
        return [dict(suggestion) for suggestion in suggestions]


def get_channel_suggestion_by_id(suggestion_id):
    """
    Получить предложение по ID

    Args:
        suggestion_id (int): ID предложения

    Returns:
        dict: Информация о предложении
    """
    with get_db_connection() as conn:
...
def approve_channel_suggestion(suggestion_id):
    """
    Одобрить предложение и отправить его как сообщение в канал

    Args:
        suggestion_id (int): ID предложения

    Returns:
        dict: Информация о предложении и созданном сообщении
    """
    with get_db_connection() as conn:
        # Получить предложение с username пользователя
        suggestion = conn.execute("""
        ...
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO channel_messages (channel_id, sender, message, is_read)
            VALUES (?, ?, ?, FALSE)
        """, (suggestion['channel_id'], suggestion['sender_username'], suggestion['content']))
        message_id = cursor.lastrowid

        # Обновить статус предложения
        conn.execute("""
            UPDATE channel_suggestions SET status = 'approved' WHERE id = ?
        """, (suggestion_id,))
...
def reject_channel_suggestion(suggestion_id):
    """
    ...
    with get_db_connection() as conn:
        ...
        suggestion = conn.execute("""
            SELECT * FROM channel_suggestions WHERE id = ?
        """, (suggestion_id,)).fetchone()
        ...
        # Обновить статус предложения
        conn.execute("""
            UPDATE channel_suggestions SET status = 'rejected' WHERE id = ?
        """, (suggestion_id,))
...
def get_pending_suggestions_count(channel_id):
    """
    Получить количество ожидающих предложений для канала

    Args:
        channel_id (int): ID канала

    Returns:
        int: Количество ожидающих предложений
    """
    with get_db_connection() as conn:
        count = conn.execute("""
            SELECT COUNT(*) as count FROM channel_suggestions
