...
from utils import (
    ...
    create_channel_suggestion, get_channel_suggestions, get_channel_suggestion_by_id,
    approve_channel_suggestion, reject_channel_suggestion, get_pending_suggestions_count
)
import datetime
...
def channel_history(channel_name):
    ...
    # Параметры пагинации
    page = request.args.get('page', 1, type=int)
    per_page = request.args.get('per_page', 30, type=int)  # Уменьшил для скорости
    
    # При page=1 загружаем новые сообщения (newest_first=True)
    # При page>1 загружаем старые (для бесконечного скролла вверх)
    newest_first = (page == 1)
    
    messages = get_channel_messages(channel['id'], offset=(page-1)*per_page, limit=per_page, newest_first=newest_first)
...
def handle_message(data):
    ...
    if not room.startswith('channel_'):
        ...
        socketio.emit('receive_message', {
            'msg': message, 'message': message, 'sender': sender, 'room': room,
...
def handle_delete_message(data):
    ...
    room = data.get('room')
    sender = session.get('username')
    delete_type = data.get('delete_type', 'self')

    print(f"DEBUG delete_message: msg_id={msg_id}, room={room}, sender={sender}, delete_type={delete_type}")

    # При delete_type='self' - удаляем у себя
    if delete_type == 'self':
        try:
            with get_db_connection() as conn:
                # Пробуем добавить в поле deleted_for
                try:
                    conn.execute("ALTER TABLE messages ADD COLUMN deleted_for TEXT DEFAULT ''")
                except:
                    pass
                conn.execute("UPDATE messages SET deleted_for = COALESCE(deleted_for, '') || ? WHERE id = ?", 
                           (f"{sender},", msg_id))
                conn.commit()
                print(f"DEBUG: Marked as deleted for self")
        except Exception as e:
            print(f"ERROR: {e}")
        
        # Отправляем ТОЛЬКО СЕБЕ
        socketio.emit('message_deleted', {'msg_id': msg_id}, room=sender)
        print(f"DEBUG: Sent delete to self only")
        
    elif delete_type == 'all':
        # Удаляем у ВСЕХ - полностью удаляем из БД
        try:
            with get_db_connection() as conn:
                ...
                conn.commit()
                print(f"DEBUG: Deleted from all tables")
        except Exception as e:
            print(f"ERROR deleting: {e}")
        
        # Отправляем ВСЕМ (broadcast)
        socketio.emit('message_deleted', {'msg_id': msg_id, 'room': room}, broadcast=True)
        print(f"DEBUG: Broadcast to all")
        
    elif delete_type == 'user':
        # Удалить у ПОЛЬЗОВАТЕЛЯ (конкретного человека, не себя)
        # Получаем имя пользователя из данных (нужно передать)
        target_user = data.get('target_user')
        
        try:
            with get_db_connection() as conn:
                # Пробуем добавить в поле deleted_for для конкретного пользователя
                try:
                    conn.execute("ALTER TABLE messages ADD COLUMN deleted_for TEXT DEFAULT ''")
                except:
                    pass
                conn.execute("UPDATE messages SET deleted_for = COALESCE(deleted_for, '') || ? WHERE id = ?", 
                           (f"{target_user},", msg_id))
                conn.commit()
                print(f"DEBUG: Marked as deleted for {target_user}")
        except Exception as e:
            print(f"ERROR: {e}")
        
        # Отправляем удаление конкретному пользователю (через его username как комнату)
        socketio.emit('message_deleted', {'msg_id': msg_id}, room=target_user)
        print(f"DEBUG: Sent delete to {target_user}")
...
@app.route('/channel/<channel_name>/history')
def channel_history_api(channel_name):
    """Получить историю сообщений канала (новые сверху)"""
    channel = get_channel_by_name(channel_name)
    if not channel:
        return jsonify({'messages': [], 'error': 'Канал не найден'})
    
    # Проверить права доступа
    if 'user_id' in session:
        with get_db_connection() as conn:
            member = conn.execute("SELECT * FROM channel_members WHERE channel_id = ? AND user_id = ?", 
                                  (channel['id'], session['user_id'])).fetchone()
            if not member and channel['is_private']:
                return jsonify({'messages': [], 'error': 'Нет доступа'})
    
    # Получить последние сообщения (новые сверху)
    ...
    with get_db_connection() as conn:
        messages = conn.execute("""
            ...
            LIMIT ?
        """, (channel['id'], limit)).fetchall()
    
    # Перевернуть чтобы старые были внизу
    messages = list(reversed([dict(m) for m in messages]))
    return jsonify({'messages': messages})


# === Предложки для каналов ===

@app.route('/submit_channel_suggestion/<channel_name>', methods=['POST'])
def submit_channel_suggestion(channel_name):
    if 'user_id' not in session:
        return jsonify({'success': False, 'error': 'Не авторизован'})
    channel = get_channel_by_name(channel_name)
    if not channel:
        return jsonify({'success': False, 'error': 'Канал не найден'})
    # Проверить, что пользователь участник канала
    with get_db_connection() as conn:
        member = conn.execute("SELECT * FROM channel_members WHERE channel_id = ? AND user_id = ?", (channel['id'], session['user_id'])).fetchone()
